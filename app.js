(() => {
  ...
  function buildWorksheet(records) {
    const headings = [
      "No.", "Container Number", "Shipping Line", "Repair Date", "Damage", "Damage Qty",
      "Material Used", "Material Qty", "Material Specs", "Item Code",
      "Labor Cost", "Material Cost", "W. Wash", "Total Cost",
    ];
    const rows = [
      excelRow(1, ["CONTAINER REPAIR MATERIALS REPORT", ...Array(13).fill("")], Array(14).fill(1), 28),
      excelRow(
        2,
        [`Repair date: ${state.filters.from || "All dates"} to ${state.filters.to || "All dates"}  •  ${records.length} record${records.length === 1 ? "" : "s"}`, ...Array(13).fill("")],
        Array(14).fill(2),
        21,
      ),
      excelRow(3, headings, Array(14).fill(3), 34),
    ];
    const merges = ["A1:N1", "A2:N2"];
    const dataStyles = [5, 4, 4, 8, 4, 6, 4, 6, 4, 4, 7, 7, 7, 7];
    let rowNumber = 4;
    ...
    records.forEach((repair, recordIndex) => {
      ...
      if (recordEnd > recordStart) {
        ["A", "B", "C", "D", "K", "L", "M", "N"].forEach((column) => {
    ...
    rows.push(
      `<row r="${rowNumber}" ht="23" customHeight="1"><c r="A${rowNumber}" s="9" t="inlineStr"><is><t>GRAND TOTAL</t></is></c>${Array.from({ length: 12 }, (_, index) => `<c r="${excelColumn(index + 2)}${rowNumber}" s="9"/>`).join("")}<c r="N${rowNumber}" s="10"><f>SUM(N4:N${lastDataRow})</f><v>0</v></c></row>`,
    );
    merges.push(`A${rowNumber}:M${rowNumber}`);
...
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  ...
  <cols>
    ...
    <col min="3" max="3" width="16" customWidth="1"/><col min="4" max="4" width="14" customWidth="1"/>
    <col min="5" max="5" width="34" customWidth="1"/><col min="6" max="6" width="13" customWidth="1"/>
    <col min="7" max="7" width="30" customWidth="1"/><col min="8" max="8" width="14" customWidth="1"/>
    <col min="9" max="9" width="32" customWidth="1"/><col min="10" max="10" width="17" customWidth="1"/>
    <col min="11" max="14" width="15" customWidth="1"/>
  </cols>
  <sheetData>${rows.join("")}</sheetData>
  <autoFilter ref="A3:N${lastDataRow}"/>
