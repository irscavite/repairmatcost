(() => {
  ...
  function buildReportRows(records) {
    ...
    records.forEach((repair) => {
      (repair.damages || []).forEach((damage) => {
        ...
        materials.forEach((material) => {
          ...
          const key = `${damageName}\u0000${materialName}`;
          if (!groups.has(key)) {
            groups.set(key, { damage: damageName, damageQty: 0, material: materialName, materialQty: 0 });
          }
    ...
    return [...groups.values()]
      ...
      .sort((a, b) => a.damage.localeCompare(b.damage) || a.material.localeCompare(b.material));
  }
  ...
  function renderReport() {
    ...
    if (!state.reportRows.length) {
      elements.reportTableBody.innerHTML = '<tr><td colspan="5" class="empty-row">No repair data found for the selected period.</td></tr>';
      return;
    }
    let previousDamage = "";
  ...
  async function loadReport() {
    ...
    elements.reportTableBody.innerHTML = '<tr><td colspan="5" class="empty-row">Loading report…</td></tr>';
    try {
      ...
      state.reportRows = [];
      elements.reportSummary.innerHTML = "";
      elements.reportTableBody.innerHTML = `<tr><td colspan="5" class="empty-row">Unable to load report: ${escapeHtml(error.message || "Firebase error")}</td></tr>`;
  ...
  function buildReportWorksheet(rows) {
    ...
    const periodName = state.reportMode[0].toUpperCase() + state.reportMode.slice(1);
    const label = reportPeriodLabel(range);
    const headings = ["Date / Period", "Damage", `Total QTY (${periodName})`, "Material Used", `Total Material Used QTY (${periodName})`];
    const xmlRows = [
      excelRow(1, ["DAMAGE & MATERIAL USAGE REPORT", "", "", "", ""], [1, 1, 1, 1, 1], 28),
      excelRow(2, [`${periodName}: ${label}  •  ${state.reportRecords.length} repair record${state.reportRecords.length === 1 ? "" : "s"}`, "", "", "", ""], [2, 2, 2, 2, 2], 21),
      excelRow(3, headings, [3, 3, 3, 3, 3], 34),
    ];
    let previousDamage = "";
    rows.forEach((row, index) => {
      const firstDamageRow = row.damage !== previousDamage;
      previousDamage = row.damage;
      xmlRows.push(excelRow(index + 4, [index === 0 ? label : "", firstDamageRow ? row.damage : "", firstDamageRow ? row.damageQty : "", row.material, row.materialQty], [4, 4, 6, 4, 6]));
    });
...
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  ...
  <cols><col min="1" max="1" width="27" customWidth="1"/><col min="2" max="2" width="38" customWidth="1"/><col min="3" max="3" width="20" customWidth="1"/><col min="4" max="4" width="34" customWidth="1"/><col min="5" max="5" width="30" customWidth="1"/></cols>
  <sheetData>${xmlRows.join("")}</sheetData>
  <autoFilter ref="A3:E${lastRow}"/>
