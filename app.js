(() => {
  ...
  const materialSpecOptions = [...specByName.values()].sort((a, b) =>
  ...
  const elements = {
    ...
    soNumber: document.querySelector("#so-number"),
    ...
    reportWeek: document.querySelector("#report-week"),
    reportMonth: document.querySelector("#report-month"),
    reportDailyField: document.querySelector("#report-daily-field"),
    reportWeeklyField: document.querySelector("#report-weekly-field"),
    reportMonthlyField: document.querySelector("#report-monthly-field"),
    reportPeriodTitle: document.querySelector("#report-period-title"),
    reportRangeLabel: document.querySelector("#report-range-label"),
    reportSummary: document.querySelector("#report-summary"),
    reportTableBody: document.querySelector("#report-table-body"),
    damageTotalHeading: document.querySelector("#damage-total-heading"),
    materialTotalHeading: document.querySelector("#material-total-heading"),
    refreshReport: document.querySelector("#refresh-report"),
    exportReportExcel: document.querySelector("#export-report-excel"),
  ...
  function specOptionsHtml(selected) {
    const hasSavedOption = !selected || specByName.has(selected);
    return [
      '<option value="">Select specification</option>',
      ...materialSpecOptions.map(
        (spec) =>
      ...
      ...(hasSavedOption
        ? []
        : [
            `<option value="${escapeHtml(selected)}" selected>${escapeHtml(selected)} (saved specification)</option>`,
          ]),
    ].join("");
  }
  ...
  function materialTable(entry) {
    ...
    const rows = entry.materials
      .map((material) => {
        ...
        const specification = `<select class="spec-select" data-action="spec" data-damage-id="${entry.id}" data-material-id="${material.id}" aria-label="Material specification for ${escapeHtml(material.material)}">${specOptionsHtml(material.specName)}</select>`;
  ...
  function resetForm() {
    ...
    elements.soNumber.value = "";
    elements.repairDate.value = defaultRepairDate();
  const reportDefaultDate = parseDateInput(defaultRepairDate()) || new Date();
  elements.reportDate.value = dateInputValue(reportDefaultDate);
  elements.reportWeek.value = isoWeekValue(reportDefaultDate);
  elements.reportMonth.value = dateInputValue(reportDefaultDate).slice(0, 7);
  ...
  function buildPayload() {
    return {
      ...
      soNumber: elements.soNumber.value.trim().toUpperCase(),
  ...
  async function deleteRecord(recordId) {
    ...
    if (!repair) return;
    const label = repair.containerNumber || "this container record";
    if (!window.confirm(`Delete ${label}? This action cannot be undone.`)) return;

    try {
      await firebaseRequest(firebaseUrl(recordId), { method: "DELETE" });
      showToast("Repair record deleted from Firebase.", "success");
      state.recordsLoaded = false;
      await loadRecords();
    } catch (error) {
      setSyncStatus("Firebase: unavailable", "error");
      showToast(error.message || "Unable to delete the repair record.", "error");
    }
  }
  ...
  function editRecord(recordId) {
    ...
    elements.soNumber.value = repair.soNumber || "";
          ...
          <details class="record-card">
            <summary>
              <div class="record-main">
                <div class="record-icon" aria-label="Container ${index + 1}">#${index + 1}</div>
                <div><strong>${escapeHtml(repair.containerNumber || "No container number")}</strong><span>${escapeHtml(repair.shippingLine || "No shipping line")}${repair.soNumber ? ` · SO: ${escapeHtml(repair.soNumber)}` : ""} · ${escapeHtml(repair.repairDate || "No date")}</span></div>
              </div>
            ...
            <div class="record-toolbar">
              ...
              <div class="record-actions-inline">
                ...
                <button class="button delete" type="button" data-action="delete-record" data-id="${escapeHtml(repair.id)}">♲ Delete Record</button>
              </div>
            </div>
  ...
  function reportRange() {
    ...
    if (state.reportMode === "weekly") return isoWeekRange(elements.reportWeek.value);
    const month = /^(\d{4})-(\d{2})$/.exec(elements.reportMonth.value || "");
    if (!month) return null;
    const year = Number(month[1]);
    const monthIndex = Number(month[2]) - 1;
    return {
      from: dateInputValue(new Date(year, monthIndex, 1)),
      to: dateInputValue(new Date(year, monthIndex + 1, 0)),
    };
  }
  ...
  function reportPeriodLabel(range) {
    if (!range) return "No period selected";
    if (state.reportMode === "daily") return humanDate(range.from);
    if (state.reportMode === "weekly") return `${humanDate(range.from)} – ${humanDate(range.to)}`;
    const date = parseDateInput(range.from);
    return date ? date.toLocaleDateString(undefined, { month: "long", year: "numeric" }) : range.from;
  }

  function reportQueryUrl(range) {
    const params = new URLSearchParams();
    params.set("orderBy", JSON.stringify("repairDate"));
    params.set("startAt", JSON.stringify(range.from));
    params.set("endAt", JSON.stringify(range.to));
    return `${FIREBASE_DB_URL}/${FIREBASE_RECORDS_PATH}.json?${params.toString()}`;
  }
  ...
  function buildReportRows(records) {
    ...
    records.forEach((repair) => {
      (repair.damages || []).forEach((damage) => {
        const damageName = damage.damage || "Unspecified Damage";
        damageTotals.set(damageName, (damageTotals.get(damageName) || 0) + numberValue(damage.quantity));
        ...
        materials.forEach((material) => {
          ...
          const key = `${damageName}\u0000${materialName}\u0000${materialSpec}`;
          if (!groups.has(key)) {
            groups.set(key, { damage: damageName, damageQty: 0, material: materialName, materialSpec, materialQty: 0 });
          }
          const row = groups.get(key);
          row.materialQty += numberValue(material?.materialQuantity);
        });
      });
    });
    return [...groups.values()]
      .map((row) => ({ ...row, damageQty: damageTotals.get(row.damage) || 0 }))
      .sort((a, b) => a.damage.localeCompare(b.damage) || a.material.localeCompare(b.material) || a.materialSpec.localeCompare(b.materialSpec));
  }

  function renderReport() {
    const range = reportRange();
    const periodName = state.reportMode[0].toUpperCase() + state.reportMode.slice(1);
    const label = reportPeriodLabel(range);
    elements.reportPeriodTitle.textContent = `${periodName} Report`;
    elements.reportRangeLabel.textContent = range ? `Report period: ${label}` : "Select a period to generate the report.";
    elements.damageTotalHeading.textContent = `Total QTY (${periodName})`;
    elements.materialTotalHeading.textContent = `Total Material Used QTY (${periodName})`;
    elements.exportReportExcel.disabled = state.reportRows.length === 0;
    elements.printReport.disabled = state.reportRows.length === 0;

    const damageTypes = new Set(state.reportRows.map((row) => row.damage)).size;
    const materialTypes = new Set(state.reportRows.filter((row) => row.material !== "—").map((row) => row.material)).size;
    elements.reportSummary.innerHTML = `
      <div><span>Repair Records</span><strong>${state.reportRecords.length}</strong></div>
      <div><span>Damage Types</span><strong>${damageTypes}</strong></div>
      <div><span>Material Types</span><strong>${materialTypes}</strong></div>`;

    if (!state.reportRows.length) {
      elements.reportTableBody.innerHTML = '<tr><td colspan="6" class="empty-row">No repair data found for the selected period.</td></tr>';
      return;
    }
    let previousDamage = "";
    elements.reportTableBody.innerHTML = state.reportRows.map((row, index) => {
      const firstDamageRow = row.damage !== previousDamage;
      previousDamage = row.damage;
      return `
      <tr>
        <td>${index === 0 ? escapeHtml(label) : ""}</td>
        <td>${firstDamageRow ? escapeHtml(row.damage) : ""}</td>
        <td class="report-number">${firstDamageRow ? formatQuantity(row.damageQty) : ""}</td>
        <td>${escapeHtml(row.material)}</td>
        <td class="report-number">${formatQuantity(row.materialQty)}</td>
        <td>${escapeHtml(row.materialSpec)}</td>
      </tr>`;
    }).join("");
  }
  ...
  async function loadReport() {
    ...
    elements.reportTableBody.innerHTML = '<tr><td colspan="6" class="empty-row">Loading report…</td></tr>';
    try {
      const payload = await firebaseRequest(reportQueryUrl(range));
      state.reportRecords = recordsFromFirebase(payload);
      state.reportRows = buildReportRows(state.reportRecords);
      ...
      state.reportRows = [];
      elements.reportSummary.innerHTML = "";
      elements.reportTableBody.innerHTML = `<tr><td colspan="6" class="empty-row">Unable to load report: ${escapeHtml(error.message || "Firebase error")}</td></tr>`;
      showToast(error.message || "Unable to load report.", "error");
    } finally {
  ...
  function setReportMode(mode) {
    ...
    elements.reportDailyField.hidden = mode !== "daily";
    elements.reportWeeklyField.hidden = mode !== "weekly";
    elements.reportMonthlyField.hidden = mode !== "monthly";
    state.reportLoaded = false;
    void loadReport();
  }

  function buildReportWorksheet(rows) {
    const range = reportRange();
    const periodName = state.reportMode[0].toUpperCase() + state.reportMode.slice(1);
    const label = reportPeriodLabel(range);
    const headings = ["Date / Period", "Damage", `Total QTY (${periodName})`, "Material Used", `Total Material Used QTY (${periodName})`, "Material Specs"];
    const xmlRows = [
      excelRow(1, ["DAMAGE & MATERIAL USAGE REPORT", "", "", "", "", ""], [1, 1, 1, 1, 1, 1], 28),
      excelRow(2, [`${periodName}: ${label}  •  ${state.reportRecords.length} repair record${state.reportRecords.length === 1 ? "" : "s"}`, "", "", "", "", ""], [2, 2, 2, 2, 2, 2], 21),
      excelRow(3, headings, [3, 3, 3, 3, 3, 3], 34),
    ];
    let previousDamage = "";
    rows.forEach((row, index) => {
      const firstDamageRow = row.damage !== previousDamage;
      previousDamage = row.damage;
      xmlRows.push(excelRow(index + 4, [index === 0 ? label : "", firstDamageRow ? row.damage : "", firstDamageRow ? row.damageQty : "", row.material, row.materialQty, row.materialSpec], [4, 4, 6, 4, 6, 4]));
    });
...
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  ...
  <cols><col min="1" max="1" width="27" customWidth="1"/><col min="2" max="2" width="38" customWidth="1"/><col min="3" max="3" width="20" customWidth="1"/><col min="4" max="4" width="34" customWidth="1"/><col min="5" max="5" width="30" customWidth="1"/><col min="6" max="6" width="35" customWidth="1"/></cols>
  ...
  function groupedDataStyle(baseStyle, isFirstRow, isLastRow) {
    const topStyles = { 4: 11, 5: 12, 6: 13, 7: 14, 8: 15 };
    const bottomStyles = { 4: 16, 5: 17, 6: 18, 7: 19, 8: 20 };
    const singleRowStyles = { 4: 21, 5: 22, 6: 23, 7: 24, 8: 25 };
    if (isFirstRow && isLastRow) return singleRowStyles[baseStyle] ?? baseStyle;
    if (isFirstRow) return topStyles[baseStyle] ?? baseStyle;
    if (isLastRow) return bottomStyles[baseStyle] ?? baseStyle;
    return baseStyle;
  }

  function buildWorksheet(records) {
    const headings = [
      "No.", "Container Number", "Shipping Line", "SO Number", "Repair Date", "Damage", "Damage Qty",
      "Material Used", "Material Qty", "Material Specs", "Item Code",
      "Labor Cost", "Material Cost", "W. Wash", "Total Cost",
    ];
    const rows = [
      excelRow(1, ["CONTAINER REPAIR MATERIALS REPORT", ...Array(14).fill("")], Array(15).fill(1), 28),
      excelRow(
        2,
        [`Repair date: ${state.filters.from || "All dates"} to ${state.filters.to || "All dates"}  •  ${records.length} record${records.length === 1 ? "" : "s"}`, ...Array(14).fill("")],
        Array(15).fill(2),
        21,
      ),
      excelRow(3, headings, Array(15).fill(3), 34),
    ];
    const merges = ["A1:O1", "A2:O2"];
    const dataStyles = [5, 4, 4, 4, 8, 4, 6, 4, 6, 4, 4, 7, 7, 7, 7];
    let rowNumber = 4;

    records.forEach((repair, recordIndex) => {
      ...
      repairRows.forEach((line, index) => {
        ...
        const lastRecordRow = index === repairRows.length - 1;
        const values = [
          firstRecordRow ? recordIndex + 1 : "",
          firstRecordRow ? repair.containerNumber || "" : "",
          firstRecordRow ? repair.shippingLine || "" : "",
          firstRecordRow ? repair.soNumber || "" : "",
        ...
        const groupStyles = dataStyles.map((style) => groupedDataStyle(style, firstRecordRow, lastRecordRow));
        rows.push(excelRow(rowNumber, values, groupStyles));
        rowNumber += 1;
      });

      const recordEnd = rowNumber - 1;
      if (recordEnd > recordStart) {
        ["A", "B", "C", "D", "E", "L", "M", "N", "O"].forEach((column) => {
    ...
    rows.push(
      `<row r="${rowNumber}" ht="23" customHeight="1"><c r="A${rowNumber}" s="9" t="inlineStr"><is><t>GRAND TOTAL</t></is></c>${Array.from({ length: 13 }, (_, index) => `<c r="${excelColumn(index + 2)}${rowNumber}" s="9"/>`).join("")}<c r="O${rowNumber}" s="10"><f>SUM(O4:O${lastDataRow})</f><v>0</v></c></row>`,
    );
    merges.push(`A${rowNumber}:N${rowNumber}`);
...
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  ...
  <sheetFormatPr defaultRowHeight="19"/>
  <cols>
    <col min="1" max="1" width="6" customWidth="1"/><col min="2" max="2" width="18" customWidth="1"/>
    <col min="3" max="3" width="16" customWidth="1"/><col min="4" max="4" width="16" customWidth="1"/>
    <col min="5" max="5" width="14" customWidth="1"/><col min="6" max="6" width="34" customWidth="1"/>
    <col min="7" max="7" width="13" customWidth="1"/><col min="8" max="8" width="30" customWidth="1"/>
    <col min="9" max="9" width="14" customWidth="1"/><col min="10" max="10" width="32" customWidth="1"/>
    <col min="11" max="11" width="17" customWidth="1"/><col min="12" max="15" width="15" customWidth="1"/>
  </cols>
  <sheetData>${rows.join("")}</sheetData>
  <autoFilter ref="A3:O${lastDataRow}"/>
  ...
  function createExcelWorkbook(records, worksheetXml = null, sheetName = "Repair Records", title = "Container Repair Materials Report") {
    ...
    const files = {
      ...
      "docProps/core.xml": `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><cp:coreProperties xmlns:cp="http://schemas.openxmlformats.org/package/2006/metadata/core-properties" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:dcterms="http://purl.org/dc/terms/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"><dc:title>${escapeXml(title)}</dc:title><dc:creator>Container Repair Materials</dc:creator><dcterms:created xsi:type="dcterms:W3CDTF">${timestamp}</dcterms:created><dcterms:modified xsi:type="dcterms:W3CDTF">${timestamp}</dcterms:modified></cp:coreProperties>`,
      ...
      "xl/workbook.xml": `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets><sheet name="${escapeXml(sheetName)}" sheetId="1" r:id="rId1"/></sheets><definedNames><definedName name="_xlnm.Print_Titles" localSheetId="0">&apos;${escapeXml(sheetName)}&apos;!$1:$3</definedName></definedNames><calcPr calcId="191029" fullCalcOnLoad="1"/></workbook>`,
      "xl/_rels/workbook.xml.rels": `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet" Target="worksheets/sheet1.xml"/><Relationship Id="rId2" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/styles" Target="styles.xml"/></Relationships>`,
      "xl/styles.xml": `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><numFmts count="3"><numFmt numFmtId="164" formatCode="0.####"/><numFmt numFmtId="165" formatCode="m/d/yyyy"/><numFmt numFmtId="166" formatCode="#,##0.00"/></numFmts><fonts count="5"><font><sz val="11"/><name val="Calibri"/></font><font><b/><color rgb="FFFFFFFF"/><sz val="16"/><name val="Calibri"/></font><font><i/><color rgb="FF64748B"/><sz val="10"/><name val="Calibri"/></font><font><b/><color rgb="FFFFFFFF"/><sz val="11"/><name val="Calibri"/></font><font><b/><color rgb="FF102B43"/><sz val="11"/><name val="Calibri"/></font></fonts><fills count="6"><fill><patternFill patternType="none"/></fill><fill><patternFill patternType="gray125"/></fill><fill><patternFill patternType="solid"><fgColor rgb="FF102B43"/><bgColor indexed="64"/></patternFill></fill><fill><patternFill patternType="solid"><fgColor rgb="FF17425F"/><bgColor indexed="64"/></patternFill></fill><fill><patternFill patternType="solid"><fgColor rgb="FFF1F5F9"/><bgColor indexed="64"/></patternFill></fill><fill><patternFill patternType="solid"><fgColor rgb="FFF28C28"/><bgColor indexed="64"/></patternFill></fill></fills><borders count="5"><border><left/><right/><top/><bottom/><diagonal/></border><border><left style="medium"><color rgb="FF8F9CA8"/></left><right style="medium"><color rgb="FF8F9CA8"/></right><top style="thin"><color rgb="FFD8E0E7"/></top><bottom style="thin"><color rgb="FFD8E0E7"/></bottom><diagonal/></border><border><left style="medium"><color rgb="FF8F9CA8"/></left><right style="medium"><color rgb="FF8F9CA8"/></right><top style="medium"><color rgb="FF64748B"/></top><bottom style="thin"><color rgb="FFD8E0E7"/></bottom><diagonal/></border><border><left style="medium"><color rgb="FF8F9CA8"/></left><right style="medium"><color rgb="FF8F9CA8"/></right><top style="thin"><color rgb="FFD8E0E7"/></top><bottom style="medium"><color rgb="FF64748B"/></bottom><diagonal/></border><border><left style="medium"><color rgb="FF8F9CA8"/></left><right style="medium"><color rgb="FF8F9CA8"/></right><top style="medium"><color rgb="FF64748B"/></top><bottom style="medium"><color rgb="FF64748B"/></bottom><diagonal/></border></borders><cellStyleXfs count="1"><xf numFmtId="0" fontId="0" fillId="0" borderId="0"/></cellStyleXfs><cellXfs count="26"><xf numFmtId="0" fontId="0" fillId="0" borderId="0" xfId="0"/><xf numFmtId="0" fontId="1" fillId="2" borderId="0" xfId="0" applyFont="1" applyFill="1" applyAlignment="1"><alignment vertical="center"/></xf><xf numFmtId="0" fontId="2" fillId="0" borderId="0" xfId="0" applyFont="1" applyAlignment="1"><alignment vertical="center"/></xf><xf numFmtId="0" fontId="3" fillId="3" borderId="1" xfId="0" applyFont="1" applyFill="1" applyBorder="1" applyAlignment="1"><alignment horizontal="center" vertical="center" wrapText="1"/></xf><xf numFmtId="0" fontId="0" fillId="0" borderId="1" xfId="0" applyBorder="1" applyAlignment="1"><alignment vertical="center" wrapText="1"/></xf><xf numFmtId="0" fontId="0" fillId="0" borderId="1" xfId="0" applyBorder="1" applyAlignment="1"><alignment horizontal="center" vertical="center"/></xf><xf numFmtId="164" fontId="0" fillId="0" borderId="1" xfId="0" applyNumberFormat="1" applyBorder="1" applyAlignment="1"><alignment horizontal="center" vertical="center"/></xf><xf numFmtId="166" fontId="0" fillId="0" borderId="1" xfId="0" applyNumberFormat="1" applyBorder="1" applyAlignment="1"><alignment horizontal="right" vertical="center"/></xf><xf numFmtId="165" fontId="0" fillId="0" borderId="1" xfId="0" applyNumberFormat="1" applyBorder="1" applyAlignment="1"><alignment horizontal="center" vertical="center"/></xf><xf numFmtId="0" fontId="4" fillId="4" borderId="1" xfId="0" applyFont="1" applyFill="1" applyBorder="1" applyAlignment="1"><alignment horizontal="right" vertical="center"/></xf><xf numFmtId="166" fontId="4" fillId="5" borderId="1" xfId="0" applyNumberFormat="1" applyFont="1" applyFill="1" applyBorder="1" applyAlignment="1"><alignment horizontal="right" vertical="center"/></xf><xf numFmtId="0" fontId="0" fillId="0" borderId="2" xfId="0" applyBorder="1" applyAlignment="1"><alignment vertical="center" wrapText="1"/></xf><xf numFmtId="0" fontId="0" fillId="0" borderId="2" xfId="0" applyBorder="1" applyAlignment="1"><alignment horizontal="center" vertical="center"/></xf><xf numFmtId="164" fontId="0" fillId="0" borderId="2" xfId="0" applyNumberFormat="1" applyBorder="1" applyAlignment="1"><alignment horizontal="center" vertical="center"/></xf><xf numFmtId="166" fontId="0" fillId="0" borderId="2" xfId="0" applyNumberFormat="1" applyBorder="1" applyAlignment="1"><alignment horizontal="right" vertical="center"/></xf><xf numFmtId="165" fontId="0" fillId="0" borderId="2" xfId="0" applyNumberFormat="1" applyBorder="1" applyAlignment="1"><alignment horizontal="center" vertical="center"/></xf><xf numFmtId="0" fontId="0" fillId="0" borderId="3" xfId="0" applyBorder="1" applyAlignment="1"><alignment vertical="center" wrapText="1"/></xf><xf numFmtId="0" fontId="0" fillId="0" borderId="3" xfId="0" applyBorder="1" applyAlignment="1"><alignment horizontal="center" vertical="center"/></xf><xf numFmtId="164" fontId="0" fillId="0" borderId="3" xfId="0" applyNumberFormat="1" applyBorder="1" applyAlignment="1"><alignment horizontal="center" vertical="center"/></xf><xf numFmtId="166" fontId="0" fillId="0" borderId="3" xfId="0" applyNumberFormat="1" applyBorder="1" applyAlignment="1"><alignment horizontal="right" vertical="center"/></xf><xf numFmtId="165" fontId="0" fillId="0" borderId="3" xfId="0" applyNumberFormat="1" applyBorder="1" applyAlignment="1"><alignment horizontal="center" vertical="center"/></xf><xf numFmtId="0" fontId="0" fillId="0" borderId="4" xfId="0" applyBorder="1" applyAlignment="1"><alignment vertical="center" wrapText="1"/></xf><xf numFmtId="0" fontId="0" fillId="0" borderId="4" xfId="0" applyBorder="1" applyAlignment="1"><alignment horizontal="center" vertical="center"/></xf><xf numFmtId="164" fontId="0" fillId="0" borderId="4" xfId="0" applyNumberFormat="1" applyBorder="1" applyAlignment="1"><alignment horizontal="center" vertical="center"/></xf><xf numFmtId="166" fontId="0" fillId="0" borderId="4" xfId="0" applyNumberFormat="1" applyBorder="1" applyAlignment="1"><alignment horizontal="right" vertical="center"/></xf><xf numFmtId="165" fontId="0" fillId="0" borderId="4" xfId="0" applyNumberFormat="1" applyBorder="1" applyAlignment="1"><alignment horizontal="center" vertical="center"/></xf></cellXfs><cellStyles count="1"><cellStyle name="Normal" xfId="0" builtinId="0"/></cellStyles></styleSheet>`,
      "xl/worksheets/sheet1.xml": worksheetXml || buildWorksheet(records),
    };
  ...
  function exportReportExcel() {
    ...
    const blob = createExcelWorkbook(
      state.reportRecords,
      buildReportWorksheet(state.reportRows),
      `${periodName} Report`,
      `${periodName} Damage & Material Usage Report`,
    );
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    const range = reportRange();
    link.href = url;
    link.download = `repair-material-${state.reportMode}-report-${range.from}-to-${range.to}.xlsx`;
    document.body.appendChild(link);
    link.click();
    link.remove();
    URL.revokeObjectURL(url);
    showToast(`${periodName} report exported to Excel.`, "success");
  }
  ...
  function showTab(name) {
    ...
    if (name === "reports" && !state.reportLoaded) void loadReport();
  }
  ...
  elements.recordsList.addEventListener("click", (event) => {
    const editButton = event.target.closest("button[data-action='edit-record']");
    if (editButton) {
      editRecord(editButton.dataset.id);
      return;
    }
    const deleteButton = event.target.closest("button[data-action='delete-record']");
    if (deleteButton) void deleteRecord(deleteButton.dataset.id);
  });
  ...
  [elements.reportDate, elements.reportWeek, elements.reportMonth].forEach((input) => {
    input.addEventListener("change", () => {
      state.reportLoaded = false;
      void loadReport();
    });
  ...
  elements.exportReportExcel.addEventListener("click", exportReportExcel);
  elements.printReport.addEventListener("click", printCurrentReport);
  window.addEventListener("afterprint", () => {
    document.body.classList.remove("print-report", "print-records");
  });
